...
class AfricasTalkingGateway
{
    ...
    public function sendMessage(
        ...
        if (strlen($to_) == 0 || strlen($message_) == 0) {
            ...
            throw new AfricasTalkingGatewayException(
                "Please supply both to and message parameters"
            );
        }
        ...
        if ($this->_responseInfo["response"] == self::HTTP_CODE_CREATED) {
            $responseObject = json_decode($this->_responseInfo['body']);
        ...
        // phpcs:ignore
        throw new AfricasTalkingGatewayException($this->_responseInfo['body']);
    }
    ...
    public function fetchMessages($lastReceivedId_)
    {
        ...
        $this->_requestUrl =
            ...
            $username .
            "&lastReceivedId=" .
            intval($lastReceivedId_);

        $this->executeGet();

        if ($this->_responseInfo["response"] == self::HTTP_CODE_OK) {
            $responseObject = json_decode($this->_responseInfo['body']);
    ...
    public function createSubscription(
        ...
        if (
            strlen($phoneNumber_) == 0 ||
            strlen($shortCode_) == 0 ||
            strlen($keyword_) == 0 ||
            strlen($checkoutToken_) == 0
        ) {
        ...
        return json_decode($this->_responseInfo['body']);
    }
    ...
    public function fetchPremiumSubscriptions(
        $shortCode_,
        $keyword_,
        $lastReceivedId_ = 0
    ) {
    ...
    public function call($from_, $to_)
    {
        ...
        if (($responseObject = json_decode($this->_responseInfo['body'])) !== null) {
    ...
    public function bankPaymentCheckoutCharge(
        ...
        $this->_requestBody = wp_json_encode([
            "username" => $this->_username,
        ...
        if ($this->_responseInfo["response"] == self::HTTP_CODE_CREATED) {
            $response = json_decode($this->_responseInfo['body']);
    ...
    public function mobilePaymentB2BRequest(
        $productName_,
        $providerData_,
        $currencyCode_,
        $amount_,
        ...
        if (
            !isset($providerData_["provider"]) ||
            strlen($providerData_["provider"]) == 0
        ) {
            // phpcs:ignore
        ...
        if (
            !isset($providerData_["destinationChannel"]) ||
            strlen($providerData_["destinationChannel"]) == 0
        ) {
            // phpcs:ignore
        ...
        if (
            !isset($providerData_["destinationAccount"]) ||
            strlen($providerData_["destinationAccount"]) == 0
        ) {
            // phpcs:ignore
        ...
        if (
            !isset($providerData_["transferType"]) ||
            strlen($providerData_["transferType"]) == 0
        ) {
            // phpcs:ignore
        ...
        $this->_requestBody = wp_json_encode($params);
    ...
    private function doExecute($response)
    {
        try {
            ...
            $this->_responseInfo['body'] = $this->_responseInfo['body'];
        } catch (Exception $e) {
            throw $e;
        }
    }
